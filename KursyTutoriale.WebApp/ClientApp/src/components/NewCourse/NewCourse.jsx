  ...
  const [tags, setTags] = useState([]);
    ...
    const handleSubmit = (event) => {
      ...
      CourseService.addCourse(
        ...
        const courseID = props.setCourseId(response.data)
    ...
    /* const handleSubmit = (event) => {
      ...
      CourseService.addCourse(
        ...
        userContext.userid,
        tagsList,
        5, // parseFloat(formData.get('price')),
        formData.get('title'),
      ).then(() => {
        history.push('/courseview/8c3a9719-ace0-4250-b4d5-21648a618702');
      });
    } */
            ...
            <Col sm={9}>
                <Input 
                    ...
                    onChange={handleInputChange}>
                        <option value=""></option>
                        {tags.map((v)=><option key={v} value={v.id}>{v.name}</option>)}
                </Input>
                {tagsList.map((tag) => (
                    <Tags 
                        name = "tagsList"
